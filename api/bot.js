...
const keyboardCommandHandlers = {
  ...
  '🔍 Browse Users': async (msg) => {
    ...
    for (const doc of usersSnapshot.docs) {
      ...
      keyboard.push([
        { text: `👤 View @${name}`, callback_data: `view_profile_${userData.userId}` }
      ]);
    }
...
const callbackQueryHandlers = {
  'manage_users': async (callbackQuery) => {
    ...
    for (const doc of usersSnapshot.docs) {
      ...
      keyboard.push([
        { text: `🔍 View @${username}`, callback_data: `view_user_${userData.userId}` }
      ]);
    }
...
async function handleViewUser(callbackQuery) {
  ...
  const keyboard = {
    inline_keyboard: [
      [
        { text: '✉️ Message User', callback_data: `message_${userId}` },
        { text: profile.isActive ? '❌ Block User' : '✅ Unblock User', callback_data: `toggle_block_${userId}` }
      ],
      [
